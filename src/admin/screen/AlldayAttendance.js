...
const AllDayAttendance = () => {
    ...
    // mobile visibility state for the sidebar (overlay)
    ...
    const handleExport = async () => {
        try {
            ...
            // Department filter bhi add kar sakte ho if needed
            const departmentFilter = statusFilter === 'all' ? null : 'IT'; // ya jo chahiye

            await AttendanceApi.exportMonthlyAttendance({
                month: selectedMonth,
                year: selectedYear,
                format: 'xlsx', // ya 'csv'
    ...
    const getDateBadgeType = (date) => {
        if (isToday(date)) return { text: "Today", color: themeColors.primary };
        if (isYesterday(date)) return { text: "Yesterday", color: themeColors.warning };
        return { text: format(date, "MMM d"), color: themeColors.textSecondary };
    };
                        ...
                        <div style={{
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    background: exportLoading ? themeColors.borderLight : themeColors.primary,
                                    color: exportLoading ? themeColors.textSecondary : 'white',
                            ...
                            >
                                {exportLoading ? '⏳ Exporting...' : '📥 Export Monthly'}
                            </button>
                        </div>
                    </div>
